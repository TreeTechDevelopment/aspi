...
function Form({ modalIsOpen, make, models, closeModal, addCar, updateCar }) {
  ...
  const [coilRender, setCoilRender] = useState([Math.random().toString()]);
  ...
  useEffect(() => {
    if (JSON.stringify(context.carToEdit) !== "{}") {
      ...
      setSparkplugs(context.carToEdit.sparkPlug)
      setWiresets(context.carToEdit.wiresets)
      setBrakeshoeFront(context.carToEdit.brakeShoeFront)
      setBrakeshoeBack(context.carToEdit.brakeShoeBack)
      setCoils(context.carToEdit.coil)
      ...
      let newSparkplugRender = [];
      for (let i = 0; i < context.carToEdit.sparkPlug.length; i++) {
        newSparkplugRender.push(Math.random().toString());
      }
      setSparkplugRender(newSparkplugRender);

      let newWiresetsRender = [];
      for (let i = 0; i < context.carToEdit.wiresets.length; i++) {
        newWiresetsRender.push(Math.random().toString());
      }
      setWiresetsRender(newWiresetsRender);

      let newBrakeshoeBackRender = [];
      for (let i = 0; i < context.carToEdit.brakeShoeBack.length; i++) {
        newBrakeshoeBackRender.push(Math.random().toString());
      }
      setBrakeshoeBackRender(newBrakeshoeBackRender);

      let newBrakeShoeFrontRender = [];
      for (let i = 0; i < context.carToEdit.brakeShoeFront.length; i++) {
        newBrakeShoeFrontRender.push(Math.random().toString());
      }
      setBrakeshoeFrontRender(newBrakeShoeFrontRender);

      let newCoilRender = [];
      for (let i = 0; i < context.carToEdit.coil.length; i++) {
        newCoilRender.push(Math.random().toString());
      }
      setCoilRender(newCoilRender);
  ...
  const saveCar = (e) => {
    ...
    if (checkForm()) {
      ...
      let data = {
        ...
        cylinder,
        motor,
        airFilter: airFilters.filter( filter => filter !== "" ),
        oilFilter: oilFilters.filter( filter => filter !== "" ),
        fuelFilter: fuelFilters.filter( filter => filter !== "" ),
        cabineFilter: cabineFilters.filter( filter => filter !== "" ),
        sparkPlug: sparkplugs.filter( sparkplug => sparkplug !== "" ),
        wiresets: wiresets.filter( wireset => wireset !== "" ),
        brakeShoeFront: brakeshoeFront.filter( brakeshoe => brakeshoe !== "" ),
        brakeShoeBack: brakeshoeBack.filter( brakeshoe => brakeshoe !== "" ),
        coil: coils
      };
